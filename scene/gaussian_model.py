...
class GaussianModel:
    ...
    def __init__(self, sh_degree, optimizer_type="default"):
        ...
        self.spectral_score_accum = torch.empty(0)
        self.spectral_denom = torch.empty(0)
        self.optimizer = None
    ...
    def restore(self, model_args, training_args):
        ...
        # Restore spectral accumulators if present (backwards compatible with old checkpoints)
        if len(model_args) > 12:
            self.spectral_score_accum = model_args[12]
            self.spectral_denom = model_args[13]
    ...
    def prune_points(self, mask):
        ...
        if self.spectral_score_accum.shape[0] == valid_points_mask.shape[0]:
            self.spectral_score_accum = self.spectral_score_accum[valid_points_mask]
            self.spectral_denom = self.spectral_denom[valid_points_mask]
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, radii,
        ...
        if use_spectral and not use_spectral_shape_split and self.spectral_denom.sum() > 0:
            ...
            if s_max > 1e-8:
                spectral_scores_norm = spectral_scores / s_max
            else:
                spectral_scores_norm = spectral_scores
            # Gaussians whose spectral score exceeds threshold get a gradient boost
            spectral_boost = (spectral_scores_norm > spectral_threshold).float()
            grads = grads + spectral_boost * max_grad * 0.5
    ...
    def add_spectral_densification_stats(self, spectral_diff_map, visibility_filter, viewpoint_cam):
        """Accumulate per-Gaussian spectral entropy difference scores for densification.

        Args:
            spectral_diff_map: (1, H, W) absolute difference of local spectral entropy maps
            visibility_filter: boolean or integer mask of visible Gaussians
            viewpoint_cam: camera used to project Gaussian centers to pixel space
        """
        # Convert visibility_filter to a flat 1D boolean mask
        vis_flat = visibility_filter.view(-1)
        if vis_flat.dtype != torch.bool:
            # Integer index tensor -> convert to boolean mask
            bool_mask = torch.zeros(self.spectral_score_accum.shape[0], dtype=torch.bool, device=vis_flat.device)
            bool_mask[vis_flat] = True
            vis_flat = bool_mask

        vis_indices = vis_flat.nonzero(as_tuple=True)[0]  # 1D int indices
        if vis_indices.shape[0] == 0:
            return

        xyz = self.get_xyz.detach()[vis_indices]  # (N_vis, 3)
        H, W = spectral_diff_map.shape[-2], spectral_diff_map.shape[-1]

        # Project 3D Gaussian centers to NDC using full_proj_transform (4x4)
        ones = torch.ones(xyz.shape[0], 1, device=xyz.device, dtype=xyz.dtype)
        xyz_homo = torch.cat([xyz, ones], dim=1)  # (N_vis, 4)
        full_proj = viewpoint_cam.full_proj_transform
        if full_proj.dim() == 3:
            full_proj = full_proj.squeeze(0)  # (4, 4)
        proj = (full_proj @ xyz_homo.T).T  # (N_vis, 4)

        # Perspective divide to NDC [-1, 1]
        w = proj[:, 3:4].clamp(min=1e-6)
        ndc = proj[:, :2] / w  # (N_vis, 2)

        # Convert NDC to pixel indices
        px = ((ndc[:, 0] + 1.0) * 0.5 * W).long().clamp(0, W - 1)
        py = ((ndc[:, 1] + 1.0) * 0.5 * H).long().clamp(0, H - 1)

        # Sample spectral difference map at each Gaussian's projected position
        scores = spectral_diff_map[0, py, px]  # (N_vis,)

        # Use a full-size buffer to avoid indexing shape ambiguity
        score_buffer = torch.zeros(self.spectral_score_accum.shape[0], 1, device=scores.device)
        score_buffer[vis_indices] = scores.unsqueeze(-1)
        self.spectral_score_accum += score_buffer

        denom_buffer = torch.zeros(self.spectral_denom.shape[0], 1, device=scores.device)
        denom_buffer[vis_indices] = 1.0
        self.spectral_denom += denom_buffer
